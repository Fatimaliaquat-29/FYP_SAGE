...
def main():
    ...
    if any_clip_moves:
        ...
        print("     Every clip above measured STATIC, so one .txt can be reused per clip --")
        print("     but ONLY for boxes that do not move.")
    print()
    print("     STATIC CAMERA IS NOT THE SAME AS A STATIC SCENE. The drift check measures")
    print("     the CAMERA, not what is in front of it. In a clip containing a person, the")
    print("     camera can be perfectly still while the person walks across the room:")
    print("       - furniture boxes  -> identical every frame, safe to copy")
    print("       - the person box   -> moves every frame, MUST be drawn per frame")
    print("     Copying a person box across frames of a 'With people' clip fabricates ground")
    print("     truth, and the recall number computed from it would be meaningless.")
